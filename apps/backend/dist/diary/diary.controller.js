...
let DiaryController = class DiaryController {
    diaryService;
    usersService;
    constructor(diaryService, usersService) {
        this.diaryService = diaryService;
        this.usersService = usersService;
    }
    async create(createDiaryDto, req) {
    ...
    async update(id, updateDiaryDto, req) {
        const user = await this.usersService.findById(req.user.userId);
        if (!user || !await this.usersService.hasEditPermission(user)) {
            throw new common_1.ForbiddenException('Edit permission required');
        }
...
__decorate([
    ...
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, update_diary_dto_1.UpdateDiaryDto, Object]),
    __metadata("design:returntype", Promise)
