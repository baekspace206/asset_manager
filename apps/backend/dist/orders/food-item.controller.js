"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.FoodItemController = void 0;
const common_1 = require("@nestjs/common");
const food_item_service_1 = require("./food-item.service");
const create_food_item_dto_1 = require("./dto/create-food-item.dto");
const create_food_rating_dto_1 = require("./dto/create-food-rating.dto");
const update_food_rating_dto_1 = require("./dto/update-food-rating.dto");
const jwt_auth_guard_1 = require("../auth/jwt-auth.guard");
const users_service_1 = require("../users/users.service");
let FoodItemController = class FoodItemController {
    foodItemService;
    usersService;
    constructor(foodItemService, usersService) {
        this.foodItemService = foodItemService;
        this.usersService = usersService;
    }
    async create(createFoodItemDto, req) {
        const user = await this.usersService.findById(req.user.userId);
        if (!user || !await this.usersService.hasEditPermission(user)) {
            throw new common_1.ForbiddenException('Edit permission required');
        }
        return this.foodItemService.create(createFoodItemDto, req.user.userId);
    }
    async findAll(filter, sort) {
        switch (filter) {
            case 'completed':
                return this.foodItemService.getCompletedItems();
            case 'incomplete':
                return this.foodItemService.getIncompleteItems();
            case 'baek-rated':
                return this.foodItemService.getBaekRatedItems();
            case 'jeong-rated':
                return this.foodItemService.getJeongRatedItems();
            default:
                if (sort === 'rating-desc' || sort === 'rating-asc') {
                    const order = sort === 'rating-desc' ? 'desc' : 'asc';
                    return this.foodItemService.getItemsSortedByRating(order);
                }
                return this.foodItemService.findAllWithRatings();
        }
    }
    findOne(id) {
        return this.foodItemService.findOne(+id);
    }
    async addRating(id, createRatingDto, req) {
        const user = await this.usersService.findById(req.user.userId);
        if (!user || !await this.usersService.hasEditPermission(user)) {
            throw new common_1.ForbiddenException('Edit permission required');
        }
        createRatingDto.foodItemId = +id;
        return this.foodItemService.addRating(createRatingDto, req.user.userId);
    }
    async updateRating(id, updateRatingDto, req) {
        const user = await this.usersService.findById(req.user.userId);
        if (!user || !await this.usersService.hasEditPermission(user)) {
            throw new common_1.ForbiddenException('Edit permission required');
        }
        return this.foodItemService.updateRating(+id, updateRatingDto, req.user.userId);
    }
    async deleteRating(id, req) {
        const user = await this.usersService.findById(req.user.userId);
        if (!user || !await this.usersService.hasEditPermission(user)) {
            throw new common_1.ForbiddenException('Edit permission required');
        }
        return this.foodItemService.deleteRating(+id, req.user.userId);
    }
    async deleteFoodItem(id, req) {
        const user = await this.usersService.findById(req.user.userId);
        if (!user || !await this.usersService.hasEditPermission(user)) {
            throw new common_1.ForbiddenException('Edit permission required');
        }
        return this.foodItemService.deleteFoodItem(+id, req.user.userId);
    }
};
exports.FoodItemController = FoodItemController;
__decorate([
    (0, common_1.Post)(),
    __param(0, (0, common_1.Body)()),
    __param(1, (0, common_1.Request)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [create_food_item_dto_1.CreateFoodItemDto, Object]),
    __metadata("design:returntype", Promise)
], FoodItemController.prototype, "create", null);
__decorate([
    (0, common_1.Get)(),
    __param(0, (0, common_1.Query)('filter')),
    __param(1, (0, common_1.Query)('sort')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String]),
    __metadata("design:returntype", Promise)
], FoodItemController.prototype, "findAll", null);
__decorate([
    (0, common_1.Get)(':id'),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", void 0)
], FoodItemController.prototype, "findOne", null);
__decorate([
    (0, common_1.Post)(':id/ratings'),
    __param(0, (0, common_1.Param)('id')),
    __param(1, (0, common_1.Body)()),
    __param(2, (0, common_1.Request)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, create_food_rating_dto_1.CreateFoodRatingDto, Object]),
    __metadata("design:returntype", Promise)
], FoodItemController.prototype, "addRating", null);
__decorate([
    (0, common_1.Patch)(':id/ratings'),
    __param(0, (0, common_1.Param)('id')),
    __param(1, (0, common_1.Body)()),
    __param(2, (0, common_1.Request)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, update_food_rating_dto_1.UpdateFoodRatingDto, Object]),
    __metadata("design:returntype", Promise)
], FoodItemController.prototype, "updateRating", null);
__decorate([
    (0, common_1.Delete)(':id/ratings'),
    __param(0, (0, common_1.Param)('id')),
    __param(1, (0, common_1.Request)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, Object]),
    __metadata("design:returntype", Promise)
], FoodItemController.prototype, "deleteRating", null);
__decorate([
    (0, common_1.Delete)(':id'),
    __param(0, (0, common_1.Param)('id')),
    __param(1, (0, common_1.Request)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, Object]),
    __metadata("design:returntype", Promise)
], FoodItemController.prototype, "deleteFoodItem", null);
exports.FoodItemController = FoodItemController = __decorate([
    (0, common_1.Controller)('food-items'),
    (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard),
    __metadata("design:paramtypes", [food_item_service_1.FoodItemService,
        users_service_1.UsersService])
], FoodItemController);
//# sourceMappingURL=food-item.controller.js.map